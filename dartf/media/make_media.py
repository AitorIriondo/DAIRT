...
def scenes(bib):
    ...
    S.append((18.0, [T("Background: the methods this work builds on"),
                     ...
                     P("Vision transformers use LayerNorm, which subtracts the mean, and the identity above fails: the rotation must then run online at every layer, or the normalization must be rewritten first by folding the centering into the weights, as SliceGPT does. The vision PTQ line, PTQ4ViT, FQ ViT and RepQ ViT, worked with diagonal reparametrizations instead, which cannot mix channels.", 32, GREY, "r", 12.0)]))
